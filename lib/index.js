...
export const LOGCAT_GUIDANCE =
  '本机已安装 dsh-logcat 插件（DSH Web GUI 的安卓实机调试/逆向工作台）：自动探测本机 adb 并对已连接的调试设备附加 logcat 流；日志自动落盘（~/.dsh/logcat/logs）支持历史回溯，崩溃/ANR 自动截图快照存档（~/.dsh/logcat/crashes，可用 crash_sessions 查阅）；侧边栏「Logcat」入口打开面板（设备切换、级别/多关键词过滤、按测试包名过滤、截图、崩溃高亮、历史回溯、events 事件视图、内存搜索工作台、WiFi 无线连接、APK 安装、一键安装 adb）。' +
